...
{
public:
  _GumScriptWeakRef (JSContextRef ctx,
                     JSValueRef value,
...
{
  return new _GumScriptWeakRef (ctx, value, notify, data, data_destroy);
}

JSValueRef
_gumjs_weak_ref_get (GumScriptWeakRef * ref)
{
  return ref->get ();
}
...
GumValueHandleObserver::finalize (Handle<Unknown>,
                                  void * context)
{
  static_cast<GumScriptWeakRef *> (context)->on_finalize ();
}
