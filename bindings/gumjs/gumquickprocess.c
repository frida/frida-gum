...
static void gum_quick_thread_observer_invoke (GumQuickThreadObserver * self,
    JSValue callback, const GumThreadDetails * thread);
...
GUMJS_DEFINE_FUNCTION (gumjs_process_attach_thread_observer)
{
  ...
  GumQuickProcess * parent;
  JSValue on_added, on_renamed, on_removed;
  gboolean observe_added, observe_renamed, observe_removed;
  GumQuickThreadObserver * observer;
  ...
  if (!_gum_quick_args_parse (args, "F{onAdded?,onRenamed?,onRemoved?}",
        &on_added, &on_renamed, &on_removed))
    return JS_EXCEPTION;
  ...
  if (!observe_added && !observe_renamed && !observe_removed)
...
gum_emit_existing_thread (const GumThreadDetails * thread,
                          GumQuickThreadObserver * observer)
{
  gum_quick_thread_observer_invoke (observer, observer->on_added, thread);
...
gum_emit_renamed_thread (GumThreadRegistry * registry,
                         ...
                         GumQuickThreadObserver * observer)
{
  gum_quick_thread_observer_invoke (observer, observer->on_renamed, thread);
}
...
gum_emit_removed_thread (GumThreadRegistry * registry,
                         ...
                         GumQuickThreadObserver * observer)
{
  gum_quick_thread_observer_invoke (observer, observer->on_removed, thread);
}
                                  ...
                                  const GumThreadDetails * thread)
{
  ...
  wrapper = _gum_quick_thread_new (ctx, thread, parent->thread);
