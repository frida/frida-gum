...
TESTCASE (user_time)
{
  ...
  if (!gum_user_time_sampler_is_available ((GumUserTimeSampler *) sampler))
  {
  ...
  do_work ();
  user_time_a = gum_sampler_sample (sampler);

  do_work ();
  user_time_b = gum_sampler_sample (sampler);
...
TESTCASE (user_time_by_id_self)
{
  ...
  sampler = gum_user_time_sampler_new_with_thread_id (tid);
  if (!gum_user_time_sampler_is_available ((GumUserTimeSampler *) sampler))
  {
    g_print ("<skipping, not yet supported on this OS> ");
    return;
...
TESTCASE (user_time_by_id_other)
{
  ...
  sampler = gum_user_time_sampler_new_with_thread_id (d.id);
  if (!gum_user_time_sampler_is_available ((GumUserTimeSampler *) sampler))
  {
    g_print ("<skipping, not yet supported on this OS> ");
    return;
