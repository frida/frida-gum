...
static GumMemoryOperation gum_exceptor_infer_memory_operation (gpointer address, 
    GumCpuContext * cpu);
  ...
  switch (sig)
  {
    ...
    case SIGBUS:
      if (siginfo->si_addr == ed.address)
        ...
        md->operation = 
          gum_exceptor_infer_memory_operation (ed.address, cpu_context);
...
gum_exceptor_infer_memory_operation (gpointer address, 
                                     GumCpuContext * cpu)
{
