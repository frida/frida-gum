...
#define GUM_MAX_BACKPATCH_JMP_ADDRS 4
...
struct _GumExecBlock
{
  ...
  GumAddress backpatch_jmp_addr[GUM_MAX_BACKPATCH_JMP_ADDRS];
  GumAddress backpatch_jmp_shim_target[GUM_MAX_BACKPATCH_JMP_ADDRS];

  GumAddress ret_code_address;
  GumAddress backpatch_ret_addr;
};
...
static void gum_exec_block_backpatch_call (GumExecBlock * block,
    GumExecBlock * from, gsize code_offset, guint8 backpatch_jmp_idx);
...
static void gum_exec_block_backpatch_ret (GumExecBlock * block,
    GumExecBlock * from, gsize unused);
  ...
  switch (backpatch->type)
  {
    case GUM_BACKPATCH_CALL:
    {
      ...
      gum_exec_block_backpatch_call (block_to, block_from, call->code_offset,
          call->backpatch_jmp_idx);
      break;
    }
    case GUM_BACKPATCH_RET:
    {
      gum_exec_block_backpatch_ret (block_to, block_from, 0);
      break;
    }
                               ...
                               guint8 backpatch_jmp_idx)
{
  ...
  from->backpatch_jmp_addr[backpatch_jmp_idx] = GUM_ADDRESS (from->code_start + code_offset);

  if (gum_stalker_is_excluding (ctx->stalker, block->real_start))
  {
    from->backpatch_jmp_shim_target[backpatch_jmp_idx] = GUM_ADDRESS (block->real_start);
  }
  else
  {
    from->backpatch_jmp_shim_target[backpatch_jmp_idx] = GUM_ADDRESS (block->code_start);
  }
  ...
  if (ctx->observer != NULL)
  {
    ...
    p.call.backpatch_jmp_idx = backpatch_jmp_idx;
                              ...
                              guint8 backpatch_jmp_idx)
{
  ...
  if (code_offset == 0)
  {
    from->backpatch_jmp_addr[backpatch_jmp_idx] = GUM_ADDRESS (block->code_start);
  }
  else
  {
    from->backpatch_jmp_addr[backpatch_jmp_idx] = GUM_ADDRESS (from->code_start + code_offset);
                              ...
                              gsize unused)
{
  ...
  gum_spinlock_acquire (&ctx->code_lock);
  from->backpatch_ret_addr = GUM_ADDRESS (block->code_start);
                                       ...
                                       GumGeneratorContext * gc)
{
  ...
  {
    for (backpatch_jmp_idx = 0; backpatch_jmp_idx < GUM_MAX_BACKPATCH_JMP_ADDRS; backpatch_jmp_idx++)
    {
      if (block->backpatch_jmp_addr[backpatch_jmp_idx] == GUM_ADDRESS (NULL)) break;
    }

    g_assert (backpatch_jmp_idx <= GUM_MAX_BACKPATCH_JMP_ADDRS);

    gum_x86_writer_put_jmp_near_ptr (cw, GUM_ADDRESS (&block->backpatch_jmp_addr[backpatch_jmp_idx]));

    backpatch_shim_offset = (cw->pc - GUM_ADDRESS (block->code_start));
  ...
  if (trust_threshold >= 0)
  {
    gum_x86_writer_put_jmp_near_ptr (cw, GUM_ADDRESS (&block->backpatch_ret_addr));
                                        ...
                                        GumGeneratorContext * gc)
{
  ...
  gum_exec_block_open_prolog (block, GUM_PROLOG_MINIMAL, gc, cw);

  gum_exec_ctx_write_push_branch_target_address (block->ctx, target, gc, cw);
  gum_x86_writer_put_pop_reg (cw, GUM_THUNK_REG_ARG1);
  gum_x86_writer_put_mov_reg_address (cw, GUM_THUNK_REG_ARG0,
      GUM_ADDRESS (block->ctx));
  gum_x86_writer_put_sub_reg_imm (cw, GUM_REG_XSP,
