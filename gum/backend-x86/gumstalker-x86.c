...
#ifdef HAVE_LINUX
static gboolean gum_exec_check_elf_section (const GumElfSectionDetails * details,
    gpointer user_data);
#endif
                                       ...
                                       GumGeneratorContext * gc)
{
  ...
  else if (gum_exec_block_is_direct_jmp_to_plt_got (block, gc, &target))
  {
     ...
     * replace the JMP instruction with a CALL and then generate the usual code
     * to handle a normal `RET` instruction immediately afterwards. Therefore,
     * if these PLT sections are in an excluded range (as is quite common when
     * fuzzing an executable), then we can stop stalking the target when they
     * are reached.
     */
                                         ...
                                         GumBranchTarget * target)
{
  ...
  gum_elf_module_enumerate_sections (elf, gum_exec_check_elf_section,
      &plt_got);
...
{
  GumX86Writer * cw = gc->code_writer;
  guint8 ret_byte = 0xc3;
  cs_insn * insn;

  insn = gum_x86_reader_disassemble_instruction_at (&ret_byte);

  gum_x86_writer_put_call_address (cw, GUM_ADDRESS (target->absolute_address));

  gc->instruction->ci = insn;
