...
static void gum_process_fxsave (GumX64FpuRegs * regs);
static void gum_process_fxrestore (GumX64FpuRegs * regs);
static void gum_process_save_avx (GumX64AvxRegs * regs);
static void gum_process_restore_avx (GumX64AvxRegs * regs);
#else
static void gum_process_fxsave (GumIA32FpuRegs * regs);
static void gum_process_fxrestore (GumIA32FpuRegs * regs);
static void gum_process_save_avx (GumIA32AvxRegs * regs);
static void gum_process_restore_avx (GumIA32AvxRegs * regs);
#endif
...
# ifdef HAVE_WINDOWS
  cpu_context->rdx = (guint64) &ctx->cached_context;
  cpu_context->rcx = (guint64) ctx->user_data;
                                            ...
                                            gpointer user_data)
{
  ...
  *((gpointer *)cpu_context->esp) = ctx->callback;
