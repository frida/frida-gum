                               ...
                               gpointer user_data)
{
  ...
  thread.tid = 1;
  while (carry_on && (devctl (fd, DCMD_PROC_TIDSTATUS, &thread, sizeof (thread), NULL) == 0))
  {
                            ...
                            gpointer user_data)
{
  ...
  g_assert (devctl (fd, DCMD_PROC_PAGEDATA, &membufs, sizeof (membufs), &num_membufs) != 0);
  close(fd);

  g_assert (num_membufs > 512);
...
int gum_process_enumerate_modules_callback (const struct dl_phdr_info *info,
                               ...
                               gpointer user_data)
{
  ...
  dl_iterate_phdr(gum_process_enumerate_modules_callback, &ctx);
}
...
gum_thread_state_from_system_thread_state (gint state)
{
  switch (state)
  {
    case STATE_RUNNING: return GUM_THREAD_RUNNING;
    case STATE_CONDVAR:
    case STATE_INTR:
    case STATE_JOIN:
    case STATE_MUTEX:
    ...
    case STATE_STOPPED: return GUM_THREAD_STOPPED;
    default:
      g_assert_not_reached ();
      break;
  }
