...
gum_function_context_make_monitor_trampoline (FunctionContext * ctx)
{
  ...
  ctx->code = gum_alloc_n_pages (1, GUM_PAGE_RWX);

  /*
   * Generate on_enter trampoline
   */
  ctx->on_enter_trampoline = ctx->code;
  gum_code_writer_init (&cw, ctx->code);
