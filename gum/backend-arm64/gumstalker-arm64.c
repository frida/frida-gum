...
struct _GumExecBlock
{
  ...
  guint ic_entry_idx;
  GumIcEntry * ic_entries[GUM_NUM_IC_ENTRIES];
};
...
{
  /*
   * On AARCH64, a single block may have multiple inline caches. We therefore
   * need to pass the index of which instance we need to backpatch.
   */
  guint index;
};
...
static void gum_exec_block_backpatch_inline_cache (GumExecBlock * block,
    GumExecBlock * from, gpointer from_insn, guint index);
...
static void gum_exec_block_backpatch_excluded_call (GumExecBlock * block,
    gpointer target, gpointer from_insn, guint index);
                                ...
                                const GumBackpatch * backpatch)
{
  ...
  switch (backpatch->type)
  {
    ...
    {
      const GumBackpatchInlineCache * inline_cache = &backpatch->inline_cache;
    ...
    case GUM_BACKPATCH_EXCLUDED_CALL:
    {
       ...
       */
      const GumBackpatchExcludedCall * excluded_call =
          &backpatch->excluded_call;
      gum_exec_block_backpatch_excluded_call (block_from, backpatch->to,
          from_insn, excluded_call->index);
    }
    default:
...
gum_exec_block_maybe_create_new_code_slabs (GumExecCtx * ctx)
{
  ...
  slow_slab = gum_exec_ctx_add_slow_slab (ctx, gum_slab_end (&code_slab->slab));
                                       ...
                                       guint index)
{
  ...
  /*
   * A GumExecBlock may contain multiple inline caches, the index is used to
   * select the correct one.
   */
  ic_entries = from->ic_entries[index];
                                       ...
                                       GumGeneratorContext * gc)
{
  ...
  {
    gum_arm64_writer_put_call_address_with_arguments (cws,
        GUM_ADDRESS (gum_exec_block_backpatch_inline_cache), 4,
        GUM_ARG_REGISTER, ARM64_REG_X0,
        GUM_ARG_ADDRESS, GUM_ADDRESS (block),
        GUM_ARG_ADDRESS, GUM_ADDRESS (gc->instruction->start),
        GUM_ARG_ADDRESS, GUM_ADDRESS (block->ic_entry_idx - 1));
  }
  ...
  if (target->reg != ARM64_REG_INVALID)
  {
     ...
     */
    gum_arm64_writer_put_call_address_with_arguments (cws,
        GUM_ADDRESS (gum_exec_block_backpatch_excluded_call), 4,
        GUM_ARG_ADDRESS, GUM_ADDRESS (block),
        GUM_ARG_REGISTER, ARM64_REG_X1,
        GUM_ARG_ADDRESS, GUM_ADDRESS (gc->instruction->start),
        GUM_ARG_ADDRESS, GUM_ADDRESS (block->ic_entry_idx - 1));
                                        ...
                                        guint index)
{
  ...
  stalker = ctx->stalker;
  ic_entries = block->ic_entries[index];
                                        ...
                                        arm64_reg ret_reg)
{
   ...
   */
    ...
    gum_arm64_writer_put_call_address_with_arguments (cws,
        GUM_ADDRESS (gum_exec_block_backpatch_slab), 3,
        GUM_ARG_ADDRESS, GUM_ADDRESS (block),
        GUM_ARG_REGISTER, ARM64_REG_X1,
        GUM_ARG_ADDRESS, GUM_ADDRESS (block->ic_entry_idx - 1));
  }
                                        ...
                                        GumArm64Writer * cws)
{
  ...
  g_assert (block->ic_entry_idx <= GUM_NUM_IC_ENTRIES);

  ic_entries = gum_slab_reserve (data_slab,
      gum_stalker_get_ic_entry_size (stalker));

  block->ic_entries[block->ic_entry_idx] = ic_entries;
  block->ic_entry_idx++;

  for (i = 0; i != stalker->ic_entries; i++)
  {
    ic_entries[i].real_start = NULL;
    ic_entries[i].code_start = GSIZE_TO_POINTER (empty_val);
  }
