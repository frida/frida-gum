...
gum_mips_relocator_write_one (GumMipsRelocator * self)
{
  ...
  /*
   * This section here deals with re-writing instructions from the original
   * function which have been over-written by the trampoline to redirect control
   * flow into frida. Before transferring control back to the original function,
   * we need to execute these instructions from another location in memory
   * before branching back to the tail of the original function which wasn't
   * clobbered.
   *
   ...
   * (since we have a 32 instruction stream we cannot include the whole
   * address). Given instructions in MIPS are aligned on a 32-bit boundary, the
   * low 2 bits are always clear and hence the whole offset or address can be
   * right shifted by two another 2 high bits used to increase the range.
   *
   * Now the tricky bit! The destination for the branch is likely to be too far
   * away to be reached. These instructions can only use a 18 bit signed offset
   * (16 bits are stored in the instruction since the low 2 bits are always
   * clear), a range of 128kb. But the copied code is likely to be in a page
   * somewhere else. For this reason, we can simply replace a branch instruction
   * with a jump. The destination for a jump instruction can be anywhere within
   * the same 256MB region as the origin. If more distance is required, then an
   * immediate could be loaded in a similar way to the trampoline made by
   * gum_mips_writer_put_prologue_trampoline and the JR instruction used.
   *
   * I haven't encountered any other types in my testing or usage. But there is
   * one limitation with jump instructions, they aren't conditional! So to
   ...
   * following a branch has already been fetched by the time the result of the
   * branch has been calculated and is hence executed whether the branch is
   * taken or not. It is therefore, not unusual to insert a NOP instruction
   * after the branch to avoid this. Finally, the behaviour when the processor
   * encounters two consecutive branches is undefined. The above pseudo code
   * will need updating accordingly, but the NOPs were excluded for simplicity.
   *
  ...
  switch (insn->id)
  {
    ...
    case MIPS_INS_BNEL:
       ...
       * instruction for MIPS and the range of branch instructions is +-128K.
