...
#define GUM_PROFILE_REPORT_GET_PRIVATE(o) ((o)->priv)
...
gum_profile_report_init (GumProfileReport * self)
{
  ...
  priv = GUM_PROFILE_REPORT_GET_PRIVATE (self);
  priv->thread_id_to_node_list = g_hash_table_new (g_direct_hash,
      g_direct_equal);
  priv->thread_root_nodes = g_ptr_array_new ();
}
...
gum_profile_report_finalize (GObject * object)
{
  ...
  GumProfileReportPrivate * priv = GUM_PROFILE_REPORT_GET_PRIVATE (self);
...
gum_profile_report_emit_xml (GumProfileReport * self)
{
  ...
  for (thread_idx = 0; thread_idx < priv->thread_root_nodes->len; thread_idx++)
  {
    ...
    root_nodes = g_ptr_array_index (priv->thread_root_nodes, thread_idx);

    g_string_append (xml, "<Thread>");

    for (node_idx = 0; node_idx < root_nodes->len; node_idx++)
    {
      append_node_to_xml_string (g_ptr_array_index (root_nodes, node_idx), xml);
    }
                                              ...
                                              guint thread_index)
{
  ...
  g_assert (thread_index < priv->thread_root_nodes->len);

  return g_ptr_array_index (priv->thread_root_nodes, thread_index);
}
                                             ...
                                             GumProfileReportNode * root_node)
{
  ...
  nodes = g_hash_table_lookup (priv->thread_id_to_node_list,
...
_gum_profile_report_sort (GumProfileReport * self)
{
  ...
  {
    GPtrArray * root_nodes = g_ptr_array_index (priv->thread_root_nodes, i);
